...
fitseq.data.tidy.letter <-  fitseq.data.tidy  %>% filter(day == day.number, lineage == lineage.letter)
...
data.lm.h = lm(log2(freq.norm.anc.1)~log2(Prot), data =filter(fitseq.data.tidy.letter, Promoter.Display == 'High') )
...
data.lm.l = lm(log2(freq.norm.anc.1)~log2(Prot), data =filter(fitseq.data.tidy.letter, Promoter.Display == 'Low') )
...
fitseq.data.prediction <- left_join(fitseq.data.high,fitseq.data.models,by= 'lineage') %>% 
  mutate(prediction =intercept + slope*Prot,
         ...
         pos.neg = ifelse(fit.resid > 0, 'Positive fitness residual', 'Negative fitness residual') ) %>%
  filter(!is.na(pos.neg))


x.strings <- 
  c('Rel.Codon.Freq','CAI','tAI','CDS.GC','dG','log2(salis.init)',
    'TASEP.avgRiboNum',	'TASEP.density.0',	'TASEP.bottle.neck.depth')
x.labels <- 
...
for (i in 1:length(x.labels)){
  ...
  x.label <- x.labels[i]
  print(x.label)
title <- paste('Comparing',x.label, '\nbetween positive and negative fitness residuals','\nLineage', lineage.letter)
png(paste0(result.dir,'all_pos_neg_hist_' ,x.string,'_lineage_',lineage.letter, '.png'),units="in", 
    width=20, height=12, res=70)

neg  <- fitseq.data.prediction %>%
  ungroup %>% 
  filter(pos.neg== 'Negative fitness residual') %>%
  select_(x.string) 
neg <- as.numeric( unlist(neg))  
pos  <- fitseq.data.prediction %>%
  ungroup %>% 
  filter(pos.neg== 'Positive fitness residual') %>%
  select_(x.string) 
...
dev.off()
  ...
  for (rbs in c('Strong','Mid','Weak')){
    print(rbs)
    pos <- fitseq.data.prediction %>% 
      filter(RBS.Display == rbs, pos.neg == 'Positive fitness residual') %>%
      select(TASEP.density.0)
    pos <- pos$TASEP.density.0
    
    neg <- fitseq.data.prediction %>% 
      filter(RBS.Display == rbs, pos.neg == 'Negative fitness residual') %>%
      select(TASEP.density.0)
    neg <- neg$TASEP.density.0
    
    print(get.wilcox.string(pos,neg))
